...
class HFBertEncoder(modeling_longformer.LongformerModel):
    def __init__(self, config, coordinator_config, args):
        modeling_longformer.LongformerModel.__init__(self, config)
        assert config.hidden_size > 0, "Encoder hidden_size can't be zero"

        if args.use_coordinator:
            self.coordinator = modeling_longformer.LongformerEncoder(coordinator_config)
    ...
    def init_encoder(cls, args, vocab_size):
        ...
        coordinator_cfg = modeling_longformer.LongformerConfig.from_pretrained(
        ...
        if args.max_seq_len > 4098:
            my_pos_embeddings = nn.Embedding(args.max_seq_len, cfg.hidden_size)
            my_pos_embeddings.weight.data[:4098] = encoder.embeddings.position_embeddings.weight.data
            n_assigned = 4098
            while n_assigned < args.max_seq_len:
                next_n_assigned = min(n_assigned+4098, args.max_seq_len)
